...
class AISender:
    ...
    async def send_imu(self, samples: Sequence[Union[ImuSample, Mapping[str, float]]]) -> None:
        """Send one `imu` request packet to Ultra96."""

        data = build_imu_data(samples)
        await self._send(MessageKind.IMU, data)

    async def send_voice_mfcc(self, features: Union[Sequence[Sequence[float]], VoiceMfccData]) -> None:
    ...
    async def send_m4a_as_mfcc(self, m4a_path: Union[str, Path], ffmpeg_path: str = "ffmpeg") -> VoiceMfccData:
    ...
    async def _send(self, kind: MessageKind, data: Union[ImuData, VoiceMfccData]) -> None:
