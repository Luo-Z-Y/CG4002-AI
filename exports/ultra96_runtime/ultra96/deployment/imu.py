...
def load_feature_norm_stats(mean_path, std_path) -> tuple[np.ndarray, np.ndarray]:
    ...
    if mean.shape != (len(FEATURE_ORDER),) or std.shape != (len(FEATURE_ORDER),):
        raise ValueError(
            f"Expected gesture normalization stats of shape ({len(FEATURE_ORDER)},), "
            f"got mean={mean.shape}, std={std.shape}"
        )
    if np.any(std == 0):
        raise ValueError("Gesture std contains zero; cannot normalise gesture window")
    return mean, std


def normalize_window(window: np.ndarray, mean: np.ndarray, std: np.ndarray) -> np.ndarray:
    arr = np.asarray(window, dtype=np.float32)
    if arr.shape == (len(FEATURE_ORDER), 60):
        arr = arr.T
    if arr.ndim != 2 or arr.shape[1] != len(FEATURE_ORDER):
        raise ValueError(f"Expected gesture window shape [T,{len(FEATURE_ORDER)}], got {arr.shape}")
